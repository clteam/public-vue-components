      ...
      <div class="vue-datepicker-panel" v-show="dayPanelIsShow">
        <div class="vue-datepicker-month">
          ...
          <div class="month-next">
            ...
            <span class="vue-datepicker-btn" @click="startChoiceMonth">{{ month + 1 }}</span>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    data() {
      let {value, conf} = {value: this.value, conf: this.conf},
...
//        }),
        let dayArr = new Array(6)
        for (let i = 0; i < 7; i++) {
          dayArr[i] = new Array(7)
        }
    ...
    methods: {
      ...
      choiceDay(d) {
        ...
        if (d && this.dateIsValid({year, month, day: d})) {
          ...
          // 如果不需要header toolbar
          if (!this.hasHeader) {
            this.$emit('hide', dateFormat(year, month, d, format))
          }
          // this.immEndChoice();
        }
      },
